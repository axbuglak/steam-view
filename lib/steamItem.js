...
class SteamItem {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(resProducts)) {
        ...
        logsProducts[key] = {
          ...
          total: (value.converted_price + value.converted_fee) / 100,
        };
      }
      this.console.log({ allProducts: logsProducts });
      try {
        ...
        const logNewProducts = {};
        for (const newProduct of Object.values(newProducts)) {
          logNewProducts[newProduct.listingid] = {
            total:
              (newProduct.converted_price + newProduct.converted_fee) / 100,
          };
        }
        this.console.log({ newCashProducts: logNewProducts });
  ...
  async buyProduct(product, float) {
    try {
      ...
      const account = this.getAccount((cvPrice + cvFee) / 100);
      if (!account) {
